...
module.exports = {
  ...
  generate: function (config) {
    ...
    if (config.template.folder) {
      files.templates.fileNames = getFileNames(config, config.template.path, config.template.lib);
    } else {
      files.templates.fileNames = [config.template.file + '.' + config.template.lib];
    }

    // Set default value on schemaSuffix if not defined
    config.schemaSuffix = config.schemaSuffix || 'json';

    // Get paths for schemas.
    files.schemas.fileNames = getFileNames(config, config.schemas, config.schemaSuffix);
